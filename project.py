...
def graph_old(filename0, filename1, filename2):
    df_mean2 = create_df(filename0, filename1, filename2)[0]

    # Select the columns 1, 3 and 5
    df_selected1 = df_mean2.iloc[:, [0, 2]]

    # Calculation of the standard deviation of each column
    std1 = df_selected1.std(numeric_only=True)

    # Select the columns 1, 3 and 5
    df_selected2 = df_mean2.iloc[:, [0, 4]]

    # Calculation of the standard deviation of each column
    std2 = df_selected2.std(numeric_only=True)

    # Creation of the bar plot
    ax = df_selected1.plot(kind='bar', x=df_selected1.columns[0], figsize=(10, 6), align='center')

    # Add the error bars of the first data set
    plt.errorbar(x=range(len(df_selected1)), y=df_selected1.iloc[:, 1], yerr=std1[0], fmt='none', ecolor='black',
                 capsize=4)

    # Add the error bars of the second data set
    plt.errorbar(x=range(len(df_selected2)), y=df_selected2.iloc[:, 1], yerr=std2[0], fmt='none', ecolor='black',
                 capsize=4)

    # Agregar el segundo conjunto de barras
    # Add the second bar plot #
    df_selected2.plot(kind='bar', x=df_selected2.columns[0], ax=ax, alpha=0.5, align='center')

    # Add title and axis tags
    plt.title(f'{filename0[:-7]}')
    plt.xlabel('Cycles')
    plt.ylabel('Force (N)')

    plt.savefig(f'{filename0[:-7]}-graph.png')
...
def graph(title,
    ...
    # Assuming the create_df() function returns a DataFrame
    ...
    # Set the legend labels for odd positions
    legend_labels = [filename0[:-7], filename3[:-7], filename6[:-7], filename9[:-7], filename12[:-7], filename15[:-7], filename18[:-7]]
    odd_positions = [True, True, True, True, True, True, True]  # Boolean list indicating odd positions
    bar_colors = ['blue', 'darkorange', 'green', 'r', 'purple', 'chocolate', 'darkturquoise']  # Colors of the bars

    # Create a custom legend with only labels for odd positions
...
def mean(filename0, filename1, filename2):
    ...
    # Round the decimals in the DataFrame to two decimal places
    df_mean_rounded = df_mean2.round(2)

    # Select only last 6 rows and columns 1, 3, and 5
    selected_columns = [0, 2, 4]
    df_selected = df_mean_rounded.iloc[:, selected_columns]

    # Display the table
    display(HTML('<h2>Averages of the 3 trials</h2>'))
    display(HTML(df_selected.to_html(index=False)))

    # Save the table as a PNG image
    plt.axis('off')  # Remove axes

    # Render the HTML table as an image
    table_html = '<table>{}</table>'.format(df_selected.to_html(index=False))
...
def std(filename0, filename1, filename2):
    ...
    display(HTML('<h2>Averages of the 3 trials</h2>'))
    display(HTML(df_std.to_html(index=False)))

    # Save the table as a PNG image
    plt.axis('off')  # Remove axes

    # Render the HTML table as an image
    table_html = '<table>{}</table>'.format(df_std.to_html(index=False))
